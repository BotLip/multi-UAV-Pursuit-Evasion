...
while ~captured(P, idx)
    e = P(idx, :);
    e_vel = e_path(T+e_step, :) - e_path(T-1+e_step, :);
    ...
    [miu_star, vncomb] = voronoiShrink(P, idx, x_max_rel, x_min_rel, y_max_rel, y_min_rel, e_vel);
...
%% Algorithm start
function [miu_star, vncomb] = voronoiShrink(P, idx, X_MAX, X_MIN, Y_MAX, Y_MIN, e_vel)
    % P : 所有无人机的位置 
    ...
    for i = 1 : size(vncomb, 2)
        miu = vncomb(i).miustar + e_vel;
        miu = miu / sqrt(sum(miu.^2));
...
%             miu_star{i} = dir;
            [miu_star{i}, ~, ~] = voronoiShrink_1v1([P(idx, :); P(i, :)], 1, X_MAX, X_MIN, Y_MAX, Y_MIN, e_vel);
        end
    end
...
function [ustar_p, ustar_e, vertex] = voronoiShrink_1v1(P, idx, X_MAX, X_MIN, Y_MAX, Y_MIN, e_vel)
    ...
    ustar_p = ustar_p + e_vel;
    ustar_p = ustar_p / sqrt(sum(ustar_p.^2));
